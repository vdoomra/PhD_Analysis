#include <TTree.h>
#include <TFile.h>

const int nsectors = 7;
const float pi = TMath::ACos(-1);

const char* sector[nsectors] = {"sector1", "sector2", "sector3", "sector4", "sector5" ,"sector6", "sector7"};
double phi_lo[nsectors] = {0, 2*pi/7, 4*pi/7, 6*pi/7, 8*pi/7, 10*pi/7, 12*pi/7};
double phi_hi[nsectors] = {2*pi/7, 4*pi/7, 6*pi/7, 8*pi/7, 10*pi/7, 12*pi/7, 2*pi};

TH1D* deltaE[nsectors] = {NULL};
TH2D* gemrphi_eloss[nsectors] = {NULL};
TH2D* gemrphi_thetaloss[nsectors] = {NULL};
TH2D* gemrphi[nsectors] = {NULL};
TH1D* delta_theta[nsectors] = {NULL};

void tilt_studies(const char* infile, const char* outfile){

    for(int i=0; i<nsectors; i++){ 
        
        deltaE[i] = new TH1D(Form("deltaE_%s", sector[i]), "Delta E Distribution", 30000, 0,3000); 
        delta_theta[i] = new TH1D(Form("delta_theta_%s", sector[i]), "Delta theta Distribution", 2000, -100, 100); 
        gemrphi[i] = new TH2D(Form("gemrphi_%s", sector[i]), "GEM r-phi Distribution", 1000, 600, 1100, 1000, 0, 7);
        gemrphi_eloss[i] = new TH2D(Form("gemrphi_eloss_%s", sector[i]), "GEM r-phi Distribution", 1000, 600, 1100, 1000, 0, 7);
        gemrphi_thetaloss[i] = new TH2D(Form("gemrphi_thetaloss_%s", sector[i]), "GEM r-phi Distribution", 1000, 600, 1100, 1000, 0, 7);
        
    } 

    TFile* fin = new TFile(infile, "READ");
    TTree* T = (TTree*)fin->Get("newT");

    int nevts = T->GetEntries();

    double front_sieve_k, behind_sieve_k, sieve_r, rate, sieve_ph, gem_r, gem_ph, front_sieve_pz, behind_sieve_pz, front_sieve_px, behind_sieve_px, front_sieve_py, behind_sieve_py;

    T->SetBranchAddress("front_sieve_k", &front_sieve_k);
    T->SetBranchAddress("front_sieve_px", &front_sieve_px);
    T->SetBranchAddress("front_sieve_py", &front_sieve_py);
    T->SetBranchAddress("front_sieve_pz", &front_sieve_pz);
    T->SetBranchAddress("behind_sieve_k", &behind_sieve_k);
    T->SetBranchAddress("behind_sieve_px", &behind_sieve_px);
    T->SetBranchAddress("behind_sieve_py", &behind_sieve_py);
    T->SetBranchAddress("behind_sieve_pz", &behind_sieve_pz);
    T->SetBranchAddress("sieve_r", &sieve_r);
    T->SetBranchAddress("sieve_ph", &sieve_ph);
    T->SetBranchAddress("rate", &rate);
    T->SetBranchAddress("gem1_r", &gem_r);
    T->SetBranchAddress("gem1_ph", &gem_ph);

    for(int ievt = 0; ievt < nevts; ievt++){

        T->GetEntry(ievt);
        if(sieve_r < 26.5) continue;
        if(sieve_ph < 0) sieve_ph += 2*pi;
        if(gem_ph < 0) gem_ph += 2*pi;
        rate = rate/200;

        double front_theta = TMath::ACos(front_sieve_pz/sqrt(front_sieve_px*front_sieve_px + front_sieve_py*front_sieve_py + front_sieve_pz*front_sieve_pz));
        double behind_theta = TMath::ACos(behind_sieve_pz/sqrt(behind_sieve_px*behind_sieve_px + behind_sieve_py*behind_sieve_py + behind_sieve_pz*behind_sieve_pz));

        for(int j=0; j<nsectors; j++){ 

            if(sieve_ph > phi_lo[j] && sieve_ph < phi_hi[j]){

                delta_theta[j]->Fill((behind_theta-front_theta)*1000, rate);
                gemrphi[j]->Fill(gem_r, gem_ph, rate); 
                deltaE[j]->Fill((front_sieve_k - behind_sieve_k), rate);
                if(fabs(front_sieve_k-behind_sieve_k) > 0.1) gemrphi_eloss[j]->Fill(gem_r, gem_ph, rate);
                if(fabs(front_theta-behind_theta)*1000 > 0.1) gemrphi_thetaloss[j]->Fill(gem_r, gem_ph, rate); 
                

            }


        }

    }

    TH1D* delta_energy_all = (TH1D*)deltaE[0]->Clone();
    TH1D* delta_theta_all = (TH1D*)delta_theta[0]->Clone();
    TH2D* gemrphi_all = (TH2D*)gemrphi[0]->Clone();

    for(int i=1; i<nsectors; i++){

        delta_energy_all->Add(deltaE[i]);
        delta_theta_all->Add(delta_theta[i]);
        gemrphi_all->Add(gemrphi[i]);

    }

    delta_energy_all->SetName("delta_energy_all_sectors");
    delta_theta_all->SetName("delta_theta_all_sectors");
    gemrphi_all->SetName("gemrphi_all_sectors");

    TFile* fout = new TFile(outfile,"RECREATE");
    fout->cd();
    for(int k=0; k<nsectors; k++){ 
       
        delta_theta[k]->Write();
        deltaE[k]->Write(); 
        gemrphi[k]->Write();
        gemrphi_eloss[k]->Write();
        gemrphi_thetaloss[k]->Write();
        
    }
    delta_theta_all->Write();
    delta_energy_all->Write();
    gemrphi_all->Write();

    fout->Close();

}